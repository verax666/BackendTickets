...
exports.findOne = (req, res) => {
    ...
    Process.findByPk(id, {
        include: [{
            model: db.subprocess, as: "subproceso",
        }]
    })
        .then(data => {
            res.send(data);
        ...
        .catch(err => {
            res.status(500).send({
                message: "Error retrieving Process with id=" + err
            });
        });
};
...
exports.findAll = (req, res) => {
    ...
    Process.findAndCountAll({
        where: condition, include: [{ model: db.subprocess, as: "subproceso" }]
    })
        .then(proceso => {
